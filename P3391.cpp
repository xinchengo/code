...
struct node
{
    ...
    void pushup() { siz = (ch[0] ? ch[0]->siz : 0) + 1 + (ch[1] ? ch[1]->siz : 0); }
    void pushdown()
    {
        if (tag)
        {
    ...
    int chsiz(int x) { return ch[x] ? ch[x]->siz : 0; }
    int find(int v) { return (v == chsiz(0)+1) ? -1 : v > chsiz(0); }
} *root = 0;
...
void splay(node *&cur, int k)
{
    ...
    splay(cur->ch[type1]->ch[type2], k-(type1?(cur->chsiz(0)+1):0)-(type2?(cur->ch[type1]->chsiz(0)+1):0));
    if (type1 == type2)
