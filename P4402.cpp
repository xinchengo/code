...
struct node
{
    ...
    bool exist(int t) { return ch[t] != nullptr; }
    int chsiz(int t) { return !exist(t) ? 0 : ch[t]->siz; }
    int find_by_size(int k) { return (k == chsiz(0) + 1) ? -1 : (k > chsiz(0)); }
    int find_min()
    {
...
void splay_kth(node *&cur, int k) // rotate the k_th element to root
{
    ...
    splay_kth(cur->ch[type1]->ch[type2], k - (type1 ? (cur->chsiz(0) + 1) : 0) - (type2 ? (cur->ch[type1]->chsiz(0) + 1) : 0));
    if(type1 == type2)
