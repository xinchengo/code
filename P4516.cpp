...
void dfs(int cur)
{
    ...
    for (auto nxt : g[cur])
    {
        ...
        for (int i = 0; i <= min(t[cur], k); i++)
            f2[i][0][0] = f[cur][i][0][0], f2[i][0][1] = f[cur][i][0][1], f2[i][1][0] = f[cur][i][1][0],
            f2[i][1][1] = f[cur][i][1][1];
        for (int i = 0; i <= min(t[cur], k); i++)
