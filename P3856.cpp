...
signed main()
{
    ...
    for (int i = 1; i <= m; i++)
        if (q[i].type[0] == 'U')
            p = pos(a[q[i].x]), add(0, p, -a[q[i].x]), add(1, p, -1), a[q[i].x] = q[i].y, p = pos(a[q[i].x]),
            add(0, p, a[q[i].x]), add(1, p, 1);
        else
            cout << ((ask(0, pos(q[i].y) - 1) >= (q[i].x - n + ask(1, pos(q[i].y) - 1)) * q[i].y) ? "TAK" : "NIE")
                 << endl;
}
